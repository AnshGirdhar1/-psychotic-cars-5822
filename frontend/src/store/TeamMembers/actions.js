...
export const getTeamMembers = (token) => async (dispatch) => {
  ...
  try {
    const res = await axios({
      method: "GET",
      url: "https://bitrix-24-website-clone.onrender.com/groups/getGroups",
      headers: {
...
export const addTeamMembers = (token, formData) => async (dispatch) => {
  ...
  try {
    await axios({
      method: "POST",
      url: "https://bitrix-24-website-clone.onrender.com/groups/create",
      data: formData,
...
export const deleteTeamMembers = (token, id) => async (dispatch) => {
  ...
  try {
    await axios({
      method: "DELETE",
      url: `https://bitrix-24-website-clone.onrender.com/groups/delete/${id}`,
      headers: { Authorization: `Bearer ${token}` },
    });
    dispatch(getTeamMembers(token));
  } catch (e) {
