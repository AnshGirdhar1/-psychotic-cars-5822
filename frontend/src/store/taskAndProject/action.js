...
const addTask = (new_task, token) => async (dispatch) => {
  ...
  try {
    await fetch("https://bitrix-24-website-clone.onrender.com/tasks/create", {
      method: "POST",
...
const getTask = (token) => async (dispatch) => {
  ...
  const r = await fetch("https://bitrix-24-website-clone.onrender.com/tasks/getTasks", {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
const deleteTask = (id, token) => async (dispatch) => {
  return await fetch(`https://bitrix-24-website-clone.onrender.com/tasks/delete/${id}`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
const updateTask = (id, token, payload) => async (dispatch) => {
  return await fetch(`https://bitrix-24-website-clone.onrender.com/tasks/edit/${id}`, {
    method: "PATCH",
